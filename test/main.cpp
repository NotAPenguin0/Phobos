...
public:
  ...
  ~GLFWWindowInterface() { glfwDestroyWindow(window); }
...
vec4 vertices[] = {vec4{-1.0, 1.0, 0.0, 1.0}, vec4{-1.0, -1.0, 0.0, 0.0},
                   vec4{1.0, -1.0, 1.0, 0.0}, vec4{-1.0, 1.0, 0, 1.0},
...
// 3. Create renderpass on main thread that does not do anything if image is not
// ready, if it is ready make it display the image
...
class TaskScheduler {
public:
  TaskScheduler(int num_threads) { threads.resize(num_threads); }
...
struct ImageWriteTask : public Task {
  ...
  ImageWriteTask(ph::Context &ctx, ph::ImageView image)
      : ctx(ctx), target_image(image) {
  ...
  void execute(uint32_t thread_index) override {
    ...
    ph::Pass image_write =
        ph::PassBuilder::create_compute("image_write_async")
            ...
            .execute([this, &itc](ph::CommandBuffer &cmd_buf) {
              ...
              VkDescriptorSet set =
                  ph::DescriptorBuilder::create(ctx,
  ...
  bool poll() override { return ctx.poll_fence(fence); }
...
int main() {
  ...
  GLFWWindowInterface *wsi =
      new GLFWWindowInterface("Phobos Test App", 800, 600);
  config.wsi = wsi;
  StdoutLogger *logger = new StdoutLogger;
  config.logger = logger;
  config.gpu_requirements.dedicated = true;
  config.gpu_requirements.min_video_memory =
      2u * 1024u * 1024u * 1024u; // 2 GB of video memory
...

  {
    ...
    ph::PipelineCreateInfo read_pci =
        ph::PipelineBuilder::create(ctx, "read")
            .add_shader("data/shaders/read.vert.spv", "main",
                        ph::ShaderStage::Vertex)
            .add_shader("data/shaders/read.frag.spv", "main",
                        ph::ShaderStage::Fragment)
            .add_vertex_input(0)
    ...
    while (wsi->is_open()) {
      ...
      ph::TypedBufferSlice<vec4> vbo =
          ifc.allocate_scratch_vbo<vec4>(sizeof(vertices));
      std::memcpy(vbo.data, vertices, sizeof(vertices));

      ph::Pass image_write =
          ph::PassBuilder::create("write")
              .add_attachment("target", ph::LoadOp::Clear,
