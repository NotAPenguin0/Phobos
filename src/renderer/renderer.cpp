...
void Renderer::render_frame(FrameInfo& info) {
    ...
    for (auto& pass : info.render_graph->passes) {
        ...
        if (!pass.draw_commands.empty()) { // TODO: Fix with proper synchronization
            update_camera_data(info, pass);
            update_materials(info, pass);
            update_lights(info, pass);
        }
        ...
        if (!pass.transforms.empty()) {
            update_model_matrices(info, pass);
        }   

        for (auto[idx, draw] : stl::enumerate(pass.draw_commands.begin(), pass.draw_commands.end())) {
            Mesh* mesh = draw.mesh;
            Material material = pass.materials[draw.material_index];
            // Bind draw data
...
void Renderer::update_camera_data(FrameInfo& info, RenderPass const& pass) {
    glm::mat4 pv = pass.projection * pass.view;
    float* data_ptr = reinterpret_cast<float*>(info.vp_ubo.ptr);
    std::memcpy(data_ptr, &pv[0][0], 16 * sizeof(float));  
    std::memcpy(data_ptr + 16, &pass.camera_pos.x, sizeof(glm::vec3));
}

void Renderer::update_model_matrices(FrameInfo& info, RenderPass const& pass) {
    info.instance_ssbo.write_data(info.fixed_descriptor_set, 
        &pass.transforms[0], pass.transforms.size() * sizeof(pass.transforms[0]));
}

void Renderer::update_materials(FrameInfo& info, RenderPass const& pass) {
    if (pass.materials.empty()) return;

    std::vector<vk::DescriptorImageInfo> img_info(pass.materials.size());
    for (size_t i = 0; i < pass.materials.size(); ++i) {
        Texture* texture = pass.materials[i].texture;
    ...
    write.descriptorCount = pass.materials.size();
    write.dstArrayElement = 0;
...
void Renderer::update_lights(FrameInfo& info, RenderPass const& pass) {
    if (pass.point_lights.empty()) return;
    // Update point lights
    std::memcpy(info.lights.ptr, &pass.point_lights[0].position.x, sizeof(PointLight) * pass.point_lights.size());
    ...
    uint32_t point_light_count = pass.point_lights.size();
    // Write data
