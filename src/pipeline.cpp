...
DescriptorBuilder& DescriptorBuilder::add_sampled_image_array(uint32_t binding, std::span<const ImageView> views, VkSampler sampler, VkImageLayout layout) {
    DescriptorBinding descr{};
    descr.binding = binding;
    descr.type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
...
DescriptorBuilder& DescriptorBuilder::add_sampled_image_array(ShaderMeta::Binding const& binding, std::span<const ImageView> views, VkSampler sampler, VkImageLayout layout) {
    return add_sampled_image_array(binding.binding, views, sampler, layout);
}

DescriptorBuilder& DescriptorBuilder::add_sampled_image_array(std::string_view binding, std::span<const ImageView> views, VkSampler sampler, VkImageLayout layout) {
